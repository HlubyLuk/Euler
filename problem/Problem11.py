...
class Coordinate(object):
    """
    Hold northeast conner of SubMatrix.
    """
    ...
    def __init__(self, x=-1, y=-1):
        ...
        if not isinstance(y, int):
            raise ArgumentError("Argument y in not int,\
                argument is instance of {}!!!", y)
        if x == -1 or y == -1:
...
class SubMatrix(object):
    """
    Subset of main matrix.
    """

    def __init__(self, coordinate=None, matrix=None):
        self.coordinate = coordinate
        self.matrix = matrix

    def get_maximum_product(self):
        main_d = self.main_diagonal()
    ...
    def main_diagonal(self):
        ...
        c = self.coordinate
        m = self.matrix
        return m[c.x][c.y] * m[c.x + 1][c.y + 1] * m[c.x + 2][c.y + 2] *\
            m[c.x + 3][c.y + 3]

    def other_diagonal(self):
        # type: () -> int
        """
        Compute product of secondary diagonal.
        :return: product of secondary diagonal.
        """
        c = self.coordinate
        m = self.matrix
        return m[c.x][c.y + 3] * m[c.x + 1][c.y + 2] * m[c.x + 2][c.y + 1] *\
            m[c.x + 3][c.y]

    def max_in_rows(self):
        # type: () -> []
        """
        Compute products of SubMatrix in rows.
        ...
        for i in range(0, limit):
            row = m[c.x + i][c.y] * m[c.x + i][c.y + 1] * m[c.x + i][c.y + 2] *\
                m[c.x + i][c.y + 3]
            ret.append(row)
        return ret
    ...
    def max_in_columns(self):
        # type: () -> []
        """
        Compute products of SubMatrix in columns.
        :return: array of products.
        """
        ret = []
        c = self.coordinate
        m = self.matrix
        for i in range(0, limit):
            cel = m[c.x][c.y + i] * m[c.x + 1][c.y + i] * m[c.x + 2][c.y + i] *\
                m[c.x + 3][c.y + i]
            ret.append(cel)
        return ret
...
class Problem11(Problem):
    ...
    def parse_matrix(self, matrix):
        """
        Spread inserted matrix into two dimensions array.
        :param matrix: to parse.
